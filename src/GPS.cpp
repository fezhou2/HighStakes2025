...
static float lastForwardPosX = getForwardOdoXPos();
static float lastForwardPosY = getForwardOdoYPos();
...
void GPS::updateGpsOdometerPos(){
  static float currForwardPosX, currForwardPosY, currHeading, currPitch;

  currForwardPosX = getForwardOdoXPos();
  currForwardPosY = getForwardOdoYPos();
  currHeading = getHeading();
  currPitch = getPitch();

  // gpsLock.writeLock();
  float factor = cos(0.5*deg2rad(lastPitch+currPitch));
  gpsPosX += factor*(currForwardPosX - lastForwardPosX) * sin(deg2rad(0.5 * (currHeading + lastHeading)));
  gpsPosY += factor*(currForwardPosX - lastForwardPosX) * cos(deg2rad(0.5 * (currHeading + lastHeading)));
  // fix sign below, might be -=
  gpsPosX -= factor*(currForwardPosY - lastForwardPosY) * cos(deg2rad(0.5 * (currHeading + lastHeading)));
  gpsPosY += factor*(currForwardPosY - lastForwardPosY) * sin(deg2rad(0.5 * (currHeading + lastHeading)));
  gpsHeading = currHeading;
  // gpsLock.writeLock();

  lastForwardPosX = currForwardPosX;
  lastForwardPosY = currForwardPosY;
  lastHeading = currHeading;
  lastPitch = currPitch;
  Brain.Screen.setCursor(9, 1);
  Brain.Screen.print("ODO_GPS:  x:  %.1f   y:  %.1f", gpsPosX, gpsPosY);
}
...
void GPS::gpsPIDMove(float _xTarget, float _yTarget, int _direct, int _maxPower, std::string mode) {
  ...
  // else we use heading for 75% distance and finish with PID forward
    ...
    posForwardAbsWithHeading(_maxPower, 0.5*dLength*_direct, _targetHeading);
